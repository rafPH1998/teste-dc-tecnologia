      ...
      <div class="flex justify-between container px-5 py-10 mx-auto border border-gray-300 mt-4 rounded bg-gray-100">
        ...
        <div class="flex flex-row-reverse items-center">
          <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="px-2 bg-gray-400 hover:bg-gray-500 rounded font-bold text-white ml-4">Sair</button>
          </form>
          <a class="text-xs text-gray-600">Bem vindo (a), {{auth()->user()->name}}</a>
